...
- 双主题跟随系统：深色为默认，浅色通过 `prefers-color-scheme: light` 自动切换（令牌定义见 `client/src/styles/global.css`）；**禁止绕过设计令牌硬编码颜色**，否则另一主题下必然出问题。
...
**色彩令牌（深色默认值；浅色对应值在 global.css 的 light 媒体查询中维护）**
...
- 根组件使用 `<van-config-provider :theme="isDark ? 'dark' : 'light'">`，`isDark` 跟随 `prefers-color-scheme`（见 App.vue）；运行时同步更新 `<meta name="theme-color">`。
- Vant 变量覆盖统一挂在 `:root`（global.css），引用设计令牌，主题切换自动跟随。
- 通过覆盖 CSS 变量对齐本规范：`--van-primary-color: var(--primary)`、`--van-background: var(--canvas)`、`--van-background-2: var(--surface-1)`、`--van-text-color: var(--ink)`、`--van-border-color: var(--hairline)`。
