...
export async function request<T = any>(
  ...
  try {
    ...
    if (!res.ok) {
      const message = data?.message || (res.status === 401 ? '未登录或会话已过期' : '请求失败')
...
export const http = {
  ...
  post: <T = any>(url: string, body?: unknown, opts?: RequestOptions) =>
    request<T>('POST', url, { ...opts, body }),
  put: <T = any>(url: string, body?: unknown, opts?: RequestOptions) =>
    request<T>('PUT', url, { ...opts, body }),
  patch: <T = any>(url: string, body?: unknown, opts?: RequestOptions) =>
    request<T>('PATCH', url, { ...opts, body }),
  delete: <T = any>(url: string, opts?: RequestOptions) => request<T>('DELETE', url, opts)
}
