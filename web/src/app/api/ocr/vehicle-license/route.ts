// 百度 OCR 识别：行驶证识别 / 车牌识别（自 Express ocr.ts 移植）
// 密钥从环境变量读取：BAIDU_OCR_API_KEY / BAIDU_OCR_SECRET_KEY
// 未配置密钥时返回明确业务错误（503），前端据此回退手输，不阻塞接车
import { NextRequest, NextResponse } from 'next/server'
import { AppError } from '@/lib/errors'
import { withAuth } from '@/lib/api-helpers'

// access_token 实例内缓存：百度签发的 token 有效期约 30 天，避免每次请求都重新鉴权
// （Serverless 每个实例各自缓存，命中即可省一次鉴权往返）
